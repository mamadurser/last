...
userRoutes.post("/getnamesusers", async (req, res) => {
  ...
  try {
    ...
    return res.json({ username: user.username, image: user.image });
  } catch (error) {
...
userRoutes.get("/followersCount", (req, res) => {
  ...
  axios
    .get(`${followuser}?following_id=${id}`)
    .then((response) => {
      // اگر داده‌ها موجود نیستند، یعنی کاربر فالوور ندارد
      if (!response.data || response.data.length === 0) {
        return res.json({ followersCount: 0 }); // به جای 404، 0 را ارسال می‌کنیم
      }

      // در صورتی که فالوورها موجود باشند، تعداد آنها را ارسال می‌کنیم
      const followersCount = response.data.length;
      res.json({ followersCount });
    })
    .catch((error) => {
      console.error("Error fetching from MockAPI:", error.message);
      // بررسی اینکه آیا خطا از سمت سرور است
      if (error.response) {
        // اگر خطا از سرور MockAPI بود، کد وضعیت آن را ارسال می‌کنیم
        return res
          .status(error.response.status)
          .json({ error: error.response.data });
      }

      // اگر خطا از نوع غیر منتظره باشد، کد 500 ارسال می‌کنیم
...
userRoutes.post("/follow", (req, res) => {
  ...
  if (!targetUserId || !currentUserId) {
    return res.status(400).json({
      success: false,
      message: "targetUserId and currentUserId are required",
    });
  }
  ...
  axios
    ...
    .then((response) => {
      ...
      const existingFollow = response.data.find(
        (follow) =>
          follow.follower_id === String(currentUserId) &&
          follow.following_id === String(targetUserId)
      );
      ...
      if (existingFollow) {
        // آنفالو کردن
        axios
          .delete(`${followuser}/${existingFollow.id}`)
          .then(() => {
  ...
  axios
    ...
    .then((response) => {
      if (response.data.length === 0) {
        ...
        axios
          .delete(`${followuser}/${followIdToDelete}`)
          .then(() => {
            res.json({ success: true, message: "Unfollowed successfully" });
          })
          .catch((err) => {
...
userRoutes.get("/following/:id", (req, res) => {
  const id = req.params.id; // گرفتن follower_id از پارامتر مسیر

  axios
    .get(`${followuser}?follower_id=${id}`) // مطمئن شوید که داده صحیح ارسال می‌شود
    .then((response) => {
      // بررسی اینکه آیا داده‌ای یافت شده است
      if (response.data.length === 0) {
        return res
          .status(404)
          .json({ error: "No following records found for this user." });
      }

      // استخراج لیست دنبال‌شده‌ها
      const followingList = response.data.map((follow) => follow.following_id);
      console.log("Following list retrieved:", followingList);

      res.json({ imfollowing: followingList }); // ارسال داده به کلاینت
    })
    .catch((error) => {
      console.error("Error fetching from MockAPI:", error.message);

      // بررسی اگر خطای پاسخ از سرور وجود دارد
      if (error.response) {
        return res
          .status(error.response.status)
          .json({ error: error.response.data || "Error from MockAPI" });
      }

      // برای خطاهای غیرقابل پیش‌بینی
      res.status(500).json({ error: "Unexpected error occurred" });
    });
});
  ...
  axios
    ...
    .then((response) => {
      ...
      if (imfollowing > 0) {
        ...
        res.status(404).json({ message: "No followers found" }); // در صورت نبود داده
      }
    })
    .catch((error) => {
