  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join_group", async (groupName) => {
      ...
      try {
        const response = await axios.get(
          `${messageApiUrl}?group_name=${groupName}`
        );
    ...
    socket.on("send_message_pv", async (data) => {
      ...
      try {
        ...
        console.error(
          "Error saving private message to Mock API:",
          error.message
        );
...
userRoutes.get("/get-chat-users", async (req, res) => {
  const userId = req.query.userId; // Retrieve userId from query
  try {
    const response = await axios.get(messageApiUrl);
    const messages = response.data;

    // Filter messages for the user
    const chatUsers = messages.filter(
      (msg) => msg.sender_id === userId || msg.receiver_id === userId
    );

    res.json(chatUsers); // Return filtered messages
  } catch (error) {
...
userRoutes.post("/add-group", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Group added successfully", data: response.data });
  } catch (error) {
    console.error("Error adding group:", error.message);
    res.status(500).json({ error: "Error adding group" });
  }
});
// userRoutes.get("/getchatusers", (req, res) => {
//   const userId = req.query.id;
//   const data = axios.get(messageApiUrl);
// });
...
userRoutes.post("/multiple", async (req, res) => {
  ...
  if (!user_ids || !Array.isArray(user_ids) || user_ids.length === 0) {
    return res
      .status(400)
      .json({ error: "Please provide a valid array of user_ids" });
  }
